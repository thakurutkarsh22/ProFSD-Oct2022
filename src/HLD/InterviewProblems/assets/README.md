...
- [`06b-design-youtube-simplified-hld.drawio`](./06b-design-youtube-simplified-hld.drawio) — **Simplified** companion diagram for [`../06b-DesignYouTube-Simplified.md`](../06b-DesignYouTube-Simplified.md). Stripped-down version of the full YouTube HLD focused on the load-bearing essentials only — 5 clients, 3 edge boxes (Multi-CDN + Origin Shield + API Gateway, plus RTMP/SRT ingest for live), 6 services (Watch / Upload / Recommendation / Engagement / View Beacon Collector / Live Transcoder), one Kafka bar with 9 topic chips, 6 storage cylinders (GCS, Spanner, Bigtable, Redis, Elasticsearch+Vespa, Feature Store), 5 async processors (Transcoding pool, Flink view-counter rollup, Flink feature streaming, Notification fan-out, CDN Pre-warmer). Includes a "6 Invariants" sticky panel and a "Top Real-World Failures" panel referencing §11 of the simplified doc. **Cut from this version:** DRM, ContentID, Search re-ranker DNN, Moderation console, Ad exchanges, Payment partners, Cold archive, Audit log, OLAP dashboards, ASR captions, multi-language audio. Use this diagram for whiteboarding in interviews; use the full `06-design-youtube-hld.drawio` for revision.
