...
- [`04-stock-broking-platform-hld.drawio`](./04-stock-broking-platform-hld.drawio) — High-Level Architecture for [`../04-StockBrokingPlatform.md`](../04-StockBrokingPlatform.md). Zerodha / Robinhood-style retail broker: API GW + WebSocket gateway, OMS + RMS + Funds (double-entry) + Portfolio, Kafka backbone, Exchange Gateway with FIX + drop-copy, Market Data multicast → fan-out, GTT / Squareoff / Settlement engines. Eight numbered, colour-coded flows on the right-hand legend.
